...
const db = mongoose.connect('mongodb://localhost/orderDb', { useNewUrlParser: true })
    ...
    orderlist.find({}, (err,data)=>{
        if(err) res.status(500).send(err)
            ...
            const orders = data.map( order => {
                //console.log("order ****** ", order)
                orderedDate = Number(order.order_date)
                sec = (now - orderedDate)/1000
